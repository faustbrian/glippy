...
The Phase 0 corpus contains project-owned source written for Glippy. It is not
...
| --- | --- | --- | --- |
| `hostile/blocks.go` | Project-authored, 2026-08-09 | Compressed block, `if` initializer, explicit ordinary-statement semicolon | Intentional Glippy layout: width-aware `if` header break |
...
| `hostile/compatibility.go` | Project-authored, 2026-08-10 | Preserved import order, numeric literal spelling, redundant parentheses, and unaligned declarations and fields | Intentional Glippy source-fidelity and layout policy; pinned output in `compatibility.gofmt.golden` |
...
alignment that Glippy deliberately does not own in the prototype dialect.
Gofmt also removes a standalone explicit empty statement and materializes an
implicit closing-label empty statement as `;`; Glippy retains whether those
