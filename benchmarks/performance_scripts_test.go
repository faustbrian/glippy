...
exit 99
`)
	writeExecutable(t, filepath.Join(toolDirectory, "uname"), `#!/bin/sh
set -eu
case "$1" in
	-s) printf '%s\n' "$FAKE_UNAME_OS" ;;
	-m) printf '%s\n' "$FAKE_UNAME_ARCH" ;;
...
`)
	writeExecutable(t, filepath.Join(toolDirectory, "git"), `#!/bin/sh
set -eu
case "$*" in
	*'rev-parse HEAD')
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !strings.Contains(string(output), test.wantError) {
				t.Fatalf("peak-rss.sh output = %q, want %q", output, test.wantError)
			}
		})
	}
}
...
`)
	...
	writeExecutable(t, fakeTime, `#!/bin/sh
set -eu
