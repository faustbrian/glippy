...
func TestPeakRSSSelection(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.wantErr == "" {
				if err != nil {
					t.Fatalf("peak-rss-select.sh error = %v, output = %q", err, output)
				}
				if string(output) != test.want {
					t.Fatalf("peak-rss-select.sh output = %q, want %q", output, test.want)
				}
				return
			}
			if err == nil {
				t.Fatalf("peak-rss-select.sh succeeded, output = %q", output)
			}
			if !strings.Contains(string(output), test.wantErr) {
				t.Fatalf("peak-rss-select.sh output = %q, want %q", output, test.wantErr)
			}
		})
	}
}
