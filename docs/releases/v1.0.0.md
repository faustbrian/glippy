...
Status: draft release notes; not bound to an exact release candidate
...
diagnostics, and editor integration through stdin/stdout and LSP. This draft
describes the intended first stable Glippy release. It is not a tag, published
release, or claim that the final exact-tree acceptance gates have passed.
...
[v0.8 exit audit](../research/v0.8-exit-audit-2026-08-26.md). Every correction
still requires parse, normalized equivalence, comment/directive identity,
idempotency, width, golden, and corpus gates before publication.
...
The final published release must bind its exact source revision to the pinned
...
Publication remains prohibited until those final candidate gates pass and the
maintainer explicitly authorizes the tag.
