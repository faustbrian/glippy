...
already present in the retained graph or can be resolved by one bounded
import-only types load, and Go internal/vendor visibility permits the root to
import it. Import-only loads use exact escaped package queries, omit test
variants, dependency syntax, and effect facts, and merge validated source
identity into the retained session. Newly resolved local imports remain
available to later edits. Base, internal-test, and external-test variants may
be retained as one bounded family and are rechecked in that order. An external
...
ambiguous test families, cgo-generated sources, unresolved or invalid import
loads, dependency overlays, file notifications, source-membership,
