...
| Rule noise | Controlled | Two-rule correctness default, per-rule admission evidence, historical 7,732-file multi-repository noise audit, and current clean Glippy plus approved `pkg/prompts` dogfood | Rerun representative default-preset dogfood on the release candidate and disposition every finding |
