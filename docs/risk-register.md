...
| Unlicensed distribution | Controlled | Gox uses the tracked SPDX `0BSD` license; release dependencies are inventoried; and the project, MIT, BSD, and Go patent notices are mandatory deterministic archive entries | Rerun native release reproducibility against the final candidate and audit linked modules before publication |
