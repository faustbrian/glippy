...
| Shared SSA | At this exit checkpoint, one run-owned SSA program mapped eligible declarations and function literals to physical source callbacks while excluding synthetic wrappers and ill-typed packages; the v0.5 memory-aware wave follow-up preserves rule sharing while bounding simultaneous root programs by selected-package count and compiled-source bytes | Proven, subsequently refined |
