...
Status: pending the final audit-bearing candidate rerun, final review, and
publication transaction.

The current candidate is
`8f52f20deea6ceb9bb316d312e26e65646ca9f6d`. CI run `33040222753`, final
fuzz run `33040254402`, and native budget/reproducibility run `33040254351`
have passed at that revision. Corpus run `33040254261` also passed all 20 jobs;
its canonical adjudication retains all 334 prior finding fingerprints, zero
false positives, and 11 accepted gaps. The uncommitted release-contract and
evidence diff still requires exact CI, native candidate reproduction, and final
review. The project therefore remains at 85% stable-v1 roadmap progress. This
record must not be changed to a 95% pre-tag decision until every pending
pre-publication row is proven or validly carried forward to one exact final
revision.
...
| Output reparses for Go 1.25 through Go 1.27 | Proven | Supported-version routing plus mandatory parse and equivalence validation in CI `33040222753` |
...
| Write mode is validated and atomic within supported limits | Proven | Native write/fix jobs on all four targets in run `33040254351` |
...
| Formatter performance meets release budgets | Proven | All four native jobs in run `33040254351` passed editor, formatter, and aggregate-memory ceilings |
...
| Rules execute only their declared analysis tier | Proven | Registry validation and syntax/types/CFG/SSA scheduler contracts in CI `33040222753` |
...
| Machine schemas are versioned | Proven | Frozen v1 machine contracts and schema references reproduced by run `33040254351` |
...
| Release archives are reproducible and checksummed | Proven for tagless candidate | Run `33040254351` reproduced four archives, manifest, checksum file, and frozen contracts across every runner |
...
| Final corpus, fuzz, race, integration, and performance gates pass | Pending | Corpus `33040254261`, fuzz `33040254402`, CI `33040222753`, and native budgets `33040254351` pass at `8f52f20`; the final audit-bearing documentation/catalog revision still requires exact CI and native reproduction |
| Real-repository dogfood is documented | Proven | Glippy self-adoption and maintainer-approved `pkg/prompts` integration |
| Publication is explicitly authorized | Proven conditionally | The maintainer granted advance release approval, effective only after all pre-publication gates pass |
