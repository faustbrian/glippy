...
The rule offers no fix. Replacing byte equality with semantic IP equality is
an intentional behavior correction rather than a semantics-preserving safe
edit. A mechanical replacement must also select a receiver, preserve comments,
and remove the `bytes` import only when no other use remains. Glippy does not
guess those ownership and import changes.
...
The diagnostic targets the complete `bytes.Equal` call. Generated files and
...
source and adds no configuration, CFG, SSA, dependency syntax, facts, or source
edits.
...
source-version selection, suppressions, generated files, type errors, and fix
absence.
...
slices as addresses. Revisit a suggestion fix when receiver selection, comment
ownership, and import cleanup can be represented as one validated transaction.
Add representation-sensitive comparisons other than `bytes.Equal` only from
an exact semantic contract and real-defect evidence.
