...
with the linked version, and identical inputs in the Darwin arm64 and Linux
arm64 environments must produce byte-identical release files.

This evidence covers artifact construction and execution. It does not prove
reproduction on a separate physical host, signing, publication, installer
behavior, Windows runtime support, network filesystem semantics, or crash
durability.
...
- Linux environment: Linux 6.12.76-linuxkit arm64 on Docker Desktop 29.6.2
  with the overlayfs containerd snapshotter on the same physical Darwin host.
...
Each environment produced the same four files byte-for-byte:
...
| `gox_v0.0.0-linux-rehearsal_manifest.json` | byte-identical across environments and checksum-valid |
| `gox_v0.0.0-linux-rehearsal_checksums.txt` | byte-identical across environments; every listed file verified |
...
Linux binary printed the same version inside the network-isolated Linux
environment and retained executable mode `0755`.

This closed the independent Docker/Linux-environment artifact rehearsal for the
then-current two-target arm64 set. Reproduction on a separate physical host,
the name decision, signing, publication, and broader platform-runtime gates
remained open.
