...
| HTTP | unreleased response bodies and other local closers | Retain `resource-not-closed` | The existing conservative ownership rule reports only local nonescaped resources and does not pretend to solve interprocedural ownership. |
...
| HTTP | general response-body ownership | Defer | Proving ownership through returns, fields, helper calls, middleware, and body replacement needs interprocedural facts beyond the current conservative local contract. |
