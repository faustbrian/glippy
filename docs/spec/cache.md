...
An effect-aware native entry MUST include the `native-effects-v6` component.
...
effect-kind bitset. It also binds every returned-state function identity, value
and error result index, and nilness under nil and non-nil error states, plus
configured must-use result indexes, blocking identities, result-to-argument
aliases, and exact cleanup-managed function result indexes inferred from
selected-module source. A dependency or project-contract change that alters
a borrow, guaranteed completion, invocation, ownership transfer, returned
relationship, must-use requirement, blocking contract, alias, or
cleanup-managed result MUST therefore
