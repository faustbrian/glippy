...
An effect-aware native entry MUST include the `native-effects-v5` component.
...
configured must-use result indexes, blocking identities, and
result-to-argument aliases. A dependency or project-contract change that alters
a borrow, guaranteed completion, invocation, ownership transfer, returned
relationship, must-use requirement, blocking contract, or alias MUST therefore
