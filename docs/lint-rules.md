...
- An exact testing FailNow, Fatal, or Fatalf call may also be followed by a final bare return, an
  unlabeled break or continue, or in a value-returning function by one zero-value variable
  declaration and a return of only those variables, without being reported; labeled branches, empty
  or initialized declarations, retained work, and lookalikes remain diagnostics.
