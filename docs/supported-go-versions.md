...
Glippy supports Go 1.25 through Go 1.27 source. The source language
is normalized to its language family, so directives such as `go 1.27.0` select
Go 1.27. Source versions older than Go 1.25 or newer than Go 1.27 fail before
...
Glippy defaults to Go 1.27. A malformed selected module or workspace file is an
error. `--stdin-filepath` uses the same resolution without reading or writing
the named source file.

Formatting and syntax-only linting validate grammar with the Go 1.27 frontend;
...
minimum. The current built-in rules support Go 1.25 through Go 1.27.
...
Installing the current Glippy source requires the toolchain selected by the
module's `go 1.27.0` directive; the Go command may choose a newer compatible toolchain
