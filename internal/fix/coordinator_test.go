...
func TestCoordinateAppliesSafeNonOverlappingFixesAndFormatsResult(t *testing.T) {
	...
	selections := []fixengine.Selection{
		selection(file, "rename-first", "rename", rules.FixSafe, edit(input, "first", "primary")),
		selection(file, "rename-second", "rename", rules.FixSafe, edit(input, "second", "secondary")),
	}
...
func TestCoordinateRejectsEveryConflictingFixButAppliesIndependentFixes(t *testing.T) {
	...
	file := loadSource(t, input)
	selections := []fixengine.Selection{
		selection(file, "replace-target-a", "replace", rules.FixSafe, edit(input, "target", "first")),
		selection(file, "replace-target-b", "replace", rules.FixSafe, edit(input, "target", "second")),
		selection(file, "rename-function", "rename", rules.FixSafe, edit(input, "run", "execute")),
	}
	result, err := fixengine.Coordinate(file, selections, fixOptions())
	if err != nil {
		t.Fatal(err)
...
func TestCoordinateRejectsACompleteMultiEditFixWhenOneEditConflicts(t *testing.T) {
	...
	result, err := fixengine.Coordinate(file, []fixengine.Selection{
		...
		selection(file, "overlap", "rewrite", rules.FixSafe, edit(input, "target", "secondary")),
	}, fixOptions())
	if err != nil {
		t.Fatal(err)
	}
	if len(result.Applied) != 0 || len(result.Rejected) != 2 || !bytes.Equal(result.Bytes, file.Bytes()) {
		t.Fatalf("Coordinate() result = %#v", result)
	}
}
...
func TestCoordinateRejectsStaleUnsafeAndInvalidSelections(t *testing.T) {
	...
	input := "package sample\nfunc run(){café()}\n"
	file := loadSource(t, input)
	stale := selection(file, "stale-rule", "rename", rules.FixSafe, edit(input, "run", "execute"))
	stale.Diagnostic.Digest = source.Digest{}
	unsafe := selection(file, "unsafe-rule", "rename", rules.FixUnsafe, edit(input, "run", "execute"))
	cafe := strings.Index(input, "café")
	invalid := selection(file, "invalid-range", "rename", rules.FixSafe, rules.Edit{
		Range:   source.Range{Start: cafe + len("caf") + 1, End: cafe + len("café")},
		NewText: "e",
	})

	result, err := fixengine.Coordinate(file, []fixengine.Selection{stale, unsafe, invalid}, fixOptions())
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(result.Bytes, file.Bytes()) || len(result.Applied) != 0 || len(result.Rejected) != 3 {
		t.Fatalf("Coordinate() result = %#v", result)
	}
...
func TestCoordinateRollsBackEveryAcceptedFixWhenValidationFails(t *testing.T) {
	...
	file := loadSource(t, input)
	result, err := fixengine.Coordinate(file, []fixengine.Selection{
		selection(file, "break-syntax", "rewrite", rules.FixSafe, edit(input, "target()", "(")),
	}, fixOptions())
	if err != nil {
		t.Fatal(err)
	}
...
func TestCoordinateAllowsBoundaryInsertionsButRejectsSameOffsetInsertions(t *testing.T) {
	...
	boundary, err := fixengine.Coordinate(file, []fixengine.Selection{
		selection(file, "replace", "rewrite", rules.FixSafe, rules.Edit{
			Range:   source.Range{Start: target, End: target + len("target")},
			NewText: "primary",
		}),
		selection(file, "suffix", "insert", rules.FixSafe, rules.Edit{
			Range:   source.Range{Start: target + len("target"), End: target + len("target")},
			NewText: "Suffix",
		}),
	}, fixOptions())
	if err != nil {
	...
	if len(boundary.Rejected) != 0 || !strings.Contains(string(boundary.Bytes), "primarySuffix()") {
		t.Fatalf("boundary insert result = %#v", boundary)
	}

	conflicting, err := fixengine.Coordinate(file, []fixengine.Selection{
		selection(file, "insert-a", "insert", rules.FixSafe, rules.Edit{
			Range:   source.Range{Start: target, End: target},
			NewText: "first",
		}),
		selection(file, "insert-b", "insert", rules.FixSafe, rules.Edit{
			Range:   source.Range{Start: target, End: target},
			NewText: "second",
		}),
	}, fixOptions())
	...
	if len(conflicting.Applied) != 0 || len(conflicting.Rejected) != 2 ||
...
func TestCoordinateRequiresExplicitSuggestionAuthorization(t *testing.T) {
	...
	input := "package sample\nfunc run(){target()}\n"
	file := loadSource(t, input)
	selected := selection(file, "suggestion", "rename", rules.FixSuggestion, edit(input, "target", "primary"))
	rejected, err := fixengine.Coordinate(file, []fixengine.Selection{selected}, fixOptions())
	if err != nil {
		t.Fatal(err)
	}
	if len(rejected.Rejected) != 1 || rejected.Rejected[0].Reason != fixengine.RejectionSuggestion {
...
func TestCoordinateIsIndependentOfSelectionOrder(t *testing.T) {
	...
	second := selection(file, "a-rule", "rename", rules.FixSafe, edit(input, "second", "secondary"))
	forward, err := fixengine.Coordinate(file, []fixengine.Selection{first, second}, fixOptions())
	if err != nil {
		t.Fatal(err)
	}
	reverse, err := fixengine.Coordinate(file, []fixengine.Selection{second, first}, fixOptions())
	if err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(forward, reverse) {
		t.Fatalf("selection order changed result:\nforward: %#v\nreverse: %#v", forward, reverse)
	}
}
...
func FuzzCoordinate(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, firstStart, firstEnd int, firstText string, secondStart, secondEnd int, secondText string) {
		input := "package sample\nfunc run(){first();second()}\n"
		file := loadSource(t, input)
		selections := []fixengine.Selection{
			selection(file, "first-rule", "rewrite", rules.FixSafe, rules.Edit{
				Range:   source.Range{Start: firstStart, End: firstEnd},
				NewText: firstText,
			}),
			selection(file, "second-rule", "rewrite", rules.FixSafe, rules.Edit{
				Range:   source.Range{Start: secondStart, End: secondEnd},
				NewText: secondText,
			}),
		}
