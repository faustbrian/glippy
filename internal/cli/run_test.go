...
func (r explainMetadataRule) Metadata() rules.Metadata {
	return r.metadata
}
...
func TestRunFormatRejectsOversizedStandardInputAsSourceFailure(t *testing.T) {
	...
	if exitCode != ExitSourceError ||
		...
		t.Fatalf(
			"Run(fmt oversized stdin) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunFormatCheckReportsOversizedFileAsIncompleteJSONSourceFailure(t *testing.T) {
	...
	exitCode := Run(
		[]string{"fmt", "--check", "--reporter=json", path},
		failingReader{},
		&stdout,
		&stderr,
	)
	...
	if report.Outcome.Category != "source_error" ||
...
func TestRunWriteFixAndCombinedCheckPreserveOversizedSource(t *testing.T) {
	...
	for _, arguments := range
		[][]string{{"fmt", "--write", path}, {"lint", "--fix", path}, {"check", path}} {
		var stdout bytes.Buffer
		var stderr bytes.Buffer
...
func TestRunTypedCombinedCheckClassifiesOversizedSource(t *testing.T) {
	...
	exitCode := Run(
		[]string{"check", "--reporter=json", root},
		failingReader{},
		&stdout,
		&stderr,
	)
	...
	if exitCode != ExitSourceError || stderr.Len() != 0 {
		t.Fatalf(
			"Run(typed oversized check) = exit %d, stderr %q",
			exitCode,
			stderr.String(),
		)
	}
	...
	if result.Outcome.Category != "source_error" ||
...
func TestRunLintReportsBuiltInDuplicateCondition(t *testing.T) {
	...
	if exitCode != ExitFindings || stderr.Len() != 0 {
		t.Fatalf(
			"Run(lint) exit = %d, stderr = %q, output = %q",
			exitCode,
			stderr.String(),
			stdout.String(),
		)
	...
	if len(result.Diagnostics) != 1 {
		t.Fatalf(
			"Run(lint) diagnostics = %#v, want one duplicate condition",
			result.Diagnostics,
		)
	}
	...
	if diagnostic.RuleID != "duplicate-condition" ||
		...
		diagnostic.Help !=
			"change the repeated condition or remove the unreachable branch" ||
	...
	if diagnostic.Range.Start != repeated ||
		diagnostic.Range.End != repeated + len("first && second") ||
...
func TestRunLintReportsBuiltInIneffectiveBreak(t *testing.T) {
	...
	if len(result.Diagnostics) != 1 {
		t.Fatalf(
			"Run(lint) diagnostics = %#v, want one ineffective break",
			result.Diagnostics,
		)
	}
	diagnostic := result.Diagnostics[0]
	...
	if diagnostic.RuleID != "ineffective-break" ||
		...
		diagnostic.Help !=
			"label the surrounding loop and break to that label, or remove the ineffective break" ||
		diagnostic.Range.Start != position ||
		diagnostic.Range.End != position + len("break") ||
...
func TestRunLintFixAppliesBuiltInRedundantBoolComparisonSafeFix(t *testing.T) {
	...
	if exitCode != ExitFindings || stderr.Len() != 0 {
		t.Fatalf(
			"Run(lint) exit = %d, stdout = %q, stderr = %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
	...
	if len(result.Diagnostics) != 1 {
		t.Fatalf(
			"Run(lint) diagnostics = %#v, want one redundant comparison",
			result.Diagnostics,
		)
	}
	diagnostic := result.Diagnostics[0]
	...
	if diagnostic.RuleID != "redundant-bool-comparison" ||
		...
		diagnostic.Range.Start != position ||
		diagnostic.Range.End != position + len("ready == true") ||
		len(diagnostic.Related) != 0 ||
		len(diagnostic.Fixes) != 1 ||
		diagnostic.Fixes[0] !=
	...
	if exitCode != ExitSuccess || stdout.Len() != 0 || stderr.Len() != 0 {
		t.Fatalf(
			"Run(second lint --fix) exit = %d, stdout = %q, stderr = %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunLintFixLeavesBuiltInDuplicateConditionUnchanged(t *testing.T) {
	...
	exitCode := Run(
		[]string{"lint", "--fix", "--reporter=json", path},
		failingReader{},
		&stdout,
		&stderr,
	)
	...
	if exitCode != ExitFindings || stderr.Len() != 0 {
		t.Fatalf(
			"Run(lint --fix) exit = %d, stderr = %q, output = %q",
			exitCode,
			stderr.String(),
			stdout.String(),
		)
...
func TestRunCombinedCheckReportsFormatAndLintFindingsWithoutMutation(t *testing.T) {
	...
	if exitCode != ExitFindings || stderr.Len() != 0 {
		t.Fatalf(
			"Run(check) exit = %d, stderr = %q, stdout = %q",
			exitCode,
			stderr.String(),
			stdout.String(),
		)
...
func TestRunCombinedCheckReportsSSAFindingAndFormatDifferenceWithoutMutation(t *testing.T) {
	...
	if err := os.WriteFile(
		filepath.Join(root, "go.mod"),
		[]byte("module example.com/checkssa\n\ngo 1.26.0\n"),
		0o600,
	);
		err != nil {
		t.Fatal(err)
	...
	stderr.Reset()
	exitCode = Run(
		[]string{"check", "--reporter=json", path},
		failingReader{},
		&stdout,
		&stderr,
	)
	if exitCode != ExitFindings || stderr.Len() != 0 {
		t.Fatalf(
			"Run(check SSA JSON) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunCombinedCheckReportsPackagePrerequisitesInJSONWithFormatting(t *testing.T) {
	...
	if err := os.WriteFile(
		filepath.Join(root, "go.mod"),
		[]byte("module example.com/checktypes\n\ngo 1.26.0\n"),
		0o600,
	);
		err != nil {
		t.Fatal(err)
	...
	if exitCode != ExitSourceError || stderr.Len() != 0 {
		t.Fatalf(
			"Run(check typed JSON) = exit %d, stderr %q, stdout %q",
			exitCode,
			stderr.String(),
			stdout.String(),
		)
	...
	if result.Outcome.Category != "source_error" ||
		result.Outcome.ExitCode != ExitSourceError ||
		!result.Summary.Complete ||
	...
	if exitCode != ExitSourceError ||
		...
		!strings.Contains(stdout.String(), "package[type]") ||
		!strings.Contains(stdout.String(), "undefined: missing") {
		t.Fatalf(
			"Run(check typed text) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunCombinedCheckReportsTypedSourceProblemsInJSON(t *testing.T) {
	...
	if err := os.WriteFile(
		filepath.Join(root, "go.mod"),
		[]byte("module example.com/checksource\n\ngo 1.26.0\n"),
		0o600,
	);
		err != nil {
		t.Fatal(err)
	...
	if exitCode != ExitSourceError || stderr.Len() != 0 {
		t.Fatalf(
			"Run(check source-problem JSON) = exit %d, stderr %q, stdout %q",
			exitCode,
			stderr.String(),
			stdout.String(),
		)
	...
	if !result.Summary.Complete ||
		...
		len(result.SourceProblems) == 0 ||
		result.SourceProblems[0].Path != path ||
		len(result.Files) != 0 {
...
func TestRunCombinedCheckReportsVersionedJSONFromOneSourceSnapshot(t *testing.T) {
	...
	if exitCode != ExitFindings || stderr.Len() != 0 {
		t.Fatalf(
			"Run(check JSON) exit = %d, stderr = %q, stdout = %q",
			exitCode,
			stderr.String(),
			stdout.String(),
		)
	...
	if len(result.Files) != 2 ||
		result.Files[0].Path != cleanPath ||
		result.Files[0].FormatStatus != "unchanged" ||
		result.Files[0].SourceDigest == "" ||
		result.Files[1].Path != changedPath ||
		result.Files[1].FormatStatus != "different" ||
		result.Files[1].SourceDigest == "" {
		t.Fatalf("Run(check JSON) files = %#v", result.Files)
	}
	if len(result.Diagnostics) != 1 ||
		result.Diagnostics[0].RuleID != "duplicate-condition" ||
		result.Diagnostics[0].Path != changedPath ||
...
func TestRunCombinedCheckReturnsSuccessSilentlyForCleanSelection(t *testing.T) {
	...
	if exitCode != ExitSuccess || stdout.Len() != 0 || stderr.Len() != 0 {
		t.Fatalf(
			"Run(check clean) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunCombinedCheckOrdersTextFindingsByPath(t *testing.T) {
	...
	if exitCode != ExitFindings || stdout.String() != want || stderr.Len() != 0 {
		t.Fatalf(
			"Run(check ordered) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunCombinedCheckSourceFailureDoesNotEmitPartialText(t *testing.T) {
	...
	if exitCode != ExitSourceError ||
		stdout.Len() != 0 ||
		!strings.Contains(stderr.String(), brokenPath) {
		t.Fatalf(
			"Run(check broken) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunCombinedCheckInvalidJSONInvocationReturnsJSON(t *testing.T) {
	...
	exitCode := Run(
		[]string{"check", "--write", "--reporter=json"},
		failingReader{},
		&stdout,
		&stderr,
	)
	...
	if result.Command != "check" ||
		...
		result.Outcome.Category != "invalid_invocation" ||
		result.Summary.Complete ||
		len(result.Files) != 0 ||
		len(result.Errors) != 1 {
...
func TestRunCombinedCheckHonorsLintAndFormatConfiguration(t *testing.T) {
	...
	if exitCode != ExitFindings ||
		stdout.String() != widthPath + ": format differs\n" ||
		stderr.Len() != 0 {
		t.Fatalf(
			"Run(check configured) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunFormatAcceptsBuiltInLintConfiguration(t *testing.T) {
	...
	if exitCode != ExitSuccess || stdout.Len() != 0 || stderr.Len() != 0 {
		t.Fatalf(
			"Run(fmt lint config) = exit %d, stdout %q, stderr %q",
			exitCode,
			stdout.String(),
			stderr.String(),
		)
...
func TestRunCombinedCheckReportsOutputFailure(t *testing.T) {
	...
	if exitCode != ExitFilesystemError ||
		...
		t.Fatalf(
			"Run(check output failure) = exit %d, stderr %q",
			exitCode,
			stderr.String(),
		)
	}
...
func TestRunCombinedCheckReportsJSONOutputFailure(t *testing.T) {
	...
	if exitCode != ExitFilesystemError ||
		!strings.Contains(stderr.String(), "write JSON report") {
		t.Fatalf(
			"Run(check JSON output failure) = exit %d, stderr %q",
			exitCode,
			stderr.String(),
		)
	}
...
func TestRunCombinedCheckReportsPreCanceledJSONInvocation(t *testing.T) {
	...
	if result.Outcome.Category != "canceled" ||
		result.Outcome.ExitCode != ExitCanceled ||
		result.Summary.Complete ||
		len(result.Files) != 0 ||
		len(result.Errors) != 1 {
...
func TestRunExplainDocumentsBuiltInDuplicateCondition(t *testing.T) {
	...
	exitCode := Run(
		[]string{"explain", "duplicate-condition"},
		failingReader{},
		&stdout,
		&stderr,
	)
...
func TestRunExplainDocumentsBuiltInRedundantBoolComparison(t *testing.T) {
	...
	exitCode := Run(
		[]string{"explain", "redundant-bool-comparison"},
		failingReader{},
		&stdout,
		&stderr,
	)
...
func TestRunExplainRejectsInvalidOrUnknownRules(t *testing.T) {
	...
	for _, arguments := range [][]string{{"explain"}, {"explain", "first", "second"}} {
		var stdout bytes.Buffer
		var stderr bytes.Buffer
		exitCode := Run(arguments, failingReader{}, &stdout, &stderr)
		if exitCode != ExitInvalidInvocation ||
			stdout.Len() != 0 ||
			stderr.String() != explainUsage {
			t.Fatalf(
				"Run(%q) = exit %d, stdout %q, stderr %q",
				arguments,
				exitCode,
...
func TestRunExplainHonorsCancellationAndOutputFailures(t *testing.T) {
	...
	if exitCode := runExplain(
		...
		t.Fatalf(
			"runExplain(canceled) = exit %d, stdout %q",
			exitCode,
			canceledOutput.String(),
		)
	}
	...
	if exitCode := runExplain(
		...
		exitCode != ExitFilesystemError ||
			!strings.Contains(stderr.String(), "write standard output") {
		t.Fatalf(
			"runExplain(failing output) = exit %d, stderr %q",
			exitCode,
			stderr.String(),
		)
	}
...
func TestRunContextRefusesCanceledInvocationBeforeReadingInput(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), context.Canceled.Error()) ||
		strings.Contains(stderr.String(), errStream.Error()) {
		t.Fatalf(
			"RunContext() stderr = %q, want cancellation without input read",
			stderr.String(),
		)
	}
}
...
func TestRunContextReportsPreCanceledJSONInvocationAsJSON(t *testing.T) {
	...
	if report.Mode != "check" ||
		report.Outcome.Category != "canceled" ||
		report.Outcome.ExitCode != ExitCanceled ||
		report.Summary.Complete ||
		len(report.Errors) != 1 {
...
func TestMapFormatTasksBoundsConcurrencyAndPreservesTaskOrder(t *testing.T) {
	...
	go func() {
		values, err := mapFormatTasks(
			context.Background(),
			tasks,
			2,
			func(_ context.Context, task formatTask) (outcome, error) {
				current := active.Add(1)
				for observed := maximum.Load();
					current > observed &&
	...
	for index, task := range tasks {
		if got.values[index].path != task.file.Path {
			t.Fatalf(
				"mapFormatTasks() result[%d] = %q, want %q",
				index,
				got.values[index].path,
				task.file.Path,
			)
		}
	}
...
func TestBoundedFormatWorkerLimitUsesEveryResourceBoundary(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(
			...
			func(t *testing.T) {
				t.Parallel()
				if got := boundedFormatWorkerLimit(
					test.resourceLimit,
					test.taskCount,
				);
					got != test.want {
					t.Fatalf(
						"boundedFormatWorkerLimit(%d, %d) = %d, want %d",
						test.resourceLimit,
						test.taskCount,
						got,
						test.want,
...
func TestMapFormatTasksChoosesFirstTaskErrorDeterministically(t *testing.T) {
	...
	_, err := mapFormatTasks(
		context.Background(),
		tasks,
		2,
		func(_ context.Context, task formatTask) (string, error) {
...
func TestRunWriteFormatsFileAndPreservesPermissions(t *testing.T) {
	...
	if stdout.Len() != 0 || stderr.Len() != 0 {
		t.Fatalf(
			"Run() stdout = %q, stderr = %q, want empty",
			stdout.String(),
			stderr.String(),
		)
	}
...
func TestRunWriteReportsVersionedJSONOutcomesInPathOrder(t *testing.T) {
	...
	exitCode := Run(
		[]string{"fmt", "--write", "--reporter=json", root},
		failingReader{},
		&stdout,
		&stderr,
	)
	...
	if report.SchemaVersion != 1 ||
		report.Command != "fmt" ||
		report.Mode != "write" ||
		report.Outcome.Category != "success" ||
		report.Outcome.ExitCode != ExitSuccess {
	...
	if report.Files[0].Path != changedPath ||
		report.Files[0].Status != "formatted" ||
		report.Files[1].Path != unchangedPath ||
		report.Files[1].Status != "unchanged" {
...
func TestRunWriteRefusesGeneratedFile(t *testing.T) {
	...
	if exitCode != ExitFilesystemError ||
		!strings.Contains(stderr.String(), "refusing to write generated file") {
...
func TestRunWriteRefusesExplicitSymlink(t *testing.T) {
	...
	if exitCode != ExitFilesystemError ||
		!strings.Contains(stderr.String(), "refusing to write symlink") {
		t.Fatalf("Run() exit = %d, stderr = %q", exitCode, stderr.String())
	}
...
func TestRunWriteValidatesEveryConfigurationBeforeAnyReplacement(t *testing.T) {
	...
	if !bytes.Equal(got, firstInput) {
		t.Fatalf(
			"Run() replaced %q before validating later configuration: %q",
			firstPath,
			got,
		)
	}
...
func TestRunFormatWriteReportsStaleSourceWithoutOverwritingNewBytes(t *testing.T) {
	...
	if exitCode != ExitConflict ||
		!strings.Contains(stderr.String(), filesystem.ErrStale.Error()) {
...
func TestRunFormatWriteCannotEscapeRootThroughParentSymlinkRace(t *testing.T) {
	...
	if exitCode != ExitConflict ||
		!strings.Contains(stderr.String(), filesystem.ErrStale.Error()) {
		t.Fatalf(
			"runFormatWrite() exit = %d, stderr = %q, want stale conflict",
			exitCode,
			stderr.String(),
		)
	}
...
func TestRunFormatWriteReportsFilesReplacedBeforeLaterConflict(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "files replaced before failure") ||
		!strings.Contains(stderr.String(), firstPath) {
		t.Fatalf(
			"runFormatWrite() stderr = %q, want prior replacement report",
			stderr.String(),
		)
	}
...
func TestRunFormatWriteReportsPartialConflictAsJSON(t *testing.T) {
	...
	report := decodeFormatJSONReport(t, stdout.Bytes())
	if report.Outcome.Category != "conflict" ||
		report.Summary.Complete ||
		report.Summary.Changed != 2 ||
		len(report.Errors) != 1 {
		t.Fatalf("runFormatWriteReported() report = %#v", report)
	}
	if report.Files[0].Path != firstPath ||
		report.Files[0].Status != "formatted" ||
		report.Files[1].Path != secondPath ||
		report.Files[1].Status != "conflict" {
...
func TestRunFormatWriteStopsBeforeNextReplacementWhenCanceled(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), context.Canceled.Error()) ||
		...
		!strings.Contains(stderr.String(), firstPath) {
		t.Fatalf(
			"runFormatWrite() stderr = %q, want cancellation and prior replacement",
			stderr.String(),
		)
	}
	second, err := os.ReadFile(secondPath)
...
func TestRunFormatWriteReportsPartialCancellationAsJSON(t *testing.T) {
	...
	report := decodeFormatJSONReport(t, stdout.Bytes())
	if report.Outcome.Category != "canceled" ||
		report.Summary.Complete ||
		report.Summary.Changed != 2 ||
		len(report.Errors) != 1 {
		t.Fatalf("runFormatWriteReported() report = %#v", report)
...
func TestRunFormatWriteReportsUncertainStateAfterReplacementError(t *testing.T) {
	...
	if !strings.Contains(
		stderr.String(),
		"files replaced or possibly replaced before failure",
	) ||
		!strings.Contains(stderr.String(), path) {
		t.Fatalf(
			"runFormatWrite() stderr = %q, want uncertain replacement report",
			stderr.String(),
		)
	}
}
...
func TestRunFormatWriteDisclosesPossiblyFormattedFileWhenJSONOutputFails(t *testing.T) {
	...
	if exitCode != ExitFilesystemError {
		t.Fatalf(
			"runFormatWriteReported() exit = %d, want %d",
			exitCode,
			ExitFilesystemError,
		)
	}
	if !strings.Contains(stderr.String(), "write JSON report") ||
		!strings.Contains(
			stderr.String(),
			"files replaced or possibly replaced before reporting failure",
		) ||
		!strings.Contains(stderr.String(), path) {
		t.Fatalf(
			"runFormatWriteReported() stderr = %q, want reporting failure and uncertain replacement",
			stderr.String(),
		)
	}
}
...
func TestRunCheckReportsDifferencesWithoutMutation(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "vendor", "ignored.go"), unformatted, 0o600);
		err != nil {
		t.Fatal(err)
	}
	var stdout bytes.Buffer
	...
	exitCode := Run(
		[]string{"fmt", "--check", "--reporter=text", root},
		failingReader{},
		&stdout,
		&stderr,
	)
...
func TestRunDiffReturnsSuccessWithoutOutputForFormattedSelection(t *testing.T) {
	...
	if stdout.Len() != 0 || stderr.Len() != 0 {
		t.Fatalf(
			"Run() stdout = %q, stderr = %q, want both empty",
			stdout.String(),
			stderr.String(),
		)
	}
...
func TestRunCheckReportsVersionedJSONOutcomesInPathOrder(t *testing.T) {
	...
	exitCode := Run(
		[]string{"fmt", "--check", "--reporter", "json", root},
		failingReader{},
		&stdout,
		&stderr,
	)
	...
	if report.SchemaVersion != 1 ||
		report.Command != "fmt" ||
		report.Mode != "check" ||
		report.Outcome.Category != "findings" ||
		report.Outcome.ExitCode != ExitFindings {
		t.Fatalf("Run() report header = %#v", report)
	}
	if report.Summary.Files != 2 ||
		report.Summary.Changed != 1 ||
	...
	if report.Files[0].Path != unchangedPath ||
		report.Files[0].Status != "unchanged" ||
		report.Files[1].Path != changedPath ||
		report.Files[1].Status != "different" {
		t.Fatalf("Run() file outcomes = %#v", report.Files)
	}
	if stderr.Len() != 0 {
		t.Fatalf("Run() stderr = %q, want empty", stderr.String())
...
func TestRunCheckReportsSourceFailureAsJSON(t *testing.T) {
	...
	report := decodeFormatJSONReport(t, stdout.Bytes())
	if report.Outcome.Category != "source_error" ||
		report.Outcome.ExitCode != ExitSourceError ||
		report.Summary.Complete ||
		len(report.Errors) != 1 {
...
func TestRunRejectsInvalidFragmentSelections(t *testing.T) {
	...
	for _, arguments := range
		...
		if !strings.Contains(
			stderr.String(),
			"--fragment=declaration|statement|expression",
		) {
			t.Fatalf(
				"Run(%q) stderr = %q, want supported fragment kinds",
				arguments,
				stderr.String(),
			)
		}
...
func TestRunReportsInvalidJSONInvocationAsJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{
			arguments: []string{"fmt", "--reporter", "json", "--unsupported"},
			mode: "invalid",
		},
		{
			arguments: []string{"fmt", "--check", "--reporter=json", "--unsupported"},
			mode: "check",
		},
		{
			arguments: []string{"fmt", "--diff", "--reporter=json", "source.go"},
			mode: "diff",
		},
		{
			arguments: []string{"fmt", "--write", "--reporter=json", "--unsupported"},
			mode: "write",
		},
		{
			arguments: []string{"fmt", "--check", "--write", "--reporter=json"},
			mode: "invalid",
		},
	}
	for _, test := range tests {
		...
		if report.Mode != test.mode ||
			report.Outcome.Category != "invalid_invocation" ||
			report.Outcome.ExitCode != ExitInvalidInvocation ||
			report.Summary.Complete ||
			len(report.Errors) != 1 {
			t.Fatalf(
				"Run(%q) report = %#v, want mode %q",
				test.arguments,
				report,
				test.mode,
			)
...
func TestRunWriteDisclosesReplacementWhenJSONOutputFails(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "write JSON report") ||
		...
		!strings.Contains(stderr.String(), path) {
		t.Fatalf(
			"Run() stderr = %q, want JSON failure and replacement disclosure",
			stderr.String(),
		)
	}
	got, err := os.ReadFile(path)
...
func TestRunRejectsMissingStreamsWithoutPanicking(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "stdout",
			stdin: bytes.NewReader([]byte(validInput)),
			stderr: &bytes.Buffer{},
		},
		{
			name: "stderr",
...
func TestRunReportsStandardOutputWriteFailures(t *testing.T) {
	...
	tests := []struct {
		name string
		stdout io.Writer
	}{{name: "error", stdout: failingWriter{}}, {name: "short write", stdout: shortWriter{}}}
	for _, test := range tests {
		t.Run(
			test.name,
			func(t *testing.T) {
				...
				if !strings.Contains(stderr.String(), "write standard output") {
					t.Fatalf(
						"Run() stderr = %q, want write failure",
						stderr.String(),
					)
				}
			},
...
func TestDiagnosticWriteFailureUsesExitSeverity(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "promotes less severe category",
			exitCode: ExitInvalidInvocation,
			want: ExitFilesystemError,
		},
		{
			name: "preserves more severe category",
