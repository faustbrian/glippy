	...
	for _, test := range tests {
		...
		t.Run(test.shell, func(t *testing.T) {
			...
			if exitCode != ExitSuccess || stderr.Len() != 0 {
				t.Fatalf("Run(completion %s) = exit %d, stderr %q", test.shell, exitCode, stderr.String())
			}
			if !strings.Contains(stdout.String(), test.marker) ||
				!strings.Contains(stdout.String(), "duplicate-condition") {
				t.Fatalf("Run(completion %s) output is incomplete:\n%s", test.shell, stdout.String())
			}
		})
	}
}
...
func TestRunCompletionPreservesCancellationAndOutputFailures(t *testing.T) {
	...
	if exitCode != ExitCanceled || stdout.Len() != 0 ||
		...
		t.Fatalf("RunContext(canceled completion) = exit %d, stdout %q, stderr %q", exitCode, stdout.String(), stderr.String())
	}
	...
	if exitCode != ExitFilesystemError ||
		...
		t.Fatalf("Run(completion output failure) = exit %d, stderr %q", exitCode, stderr.String())
	}
}
