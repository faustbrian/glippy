...
func FuzzRenderDeterministic(f *testing.F) {
	...
	f.Fuzz(
		func(t *testing.T, shape []byte, rawWidth, rawTabWidth uint8, rawBudget uint16) {
			if len(shape) > 4 << 10 {
				t.Skip()
			}
			arena := NewArena()
			...
			for offset, operation := range shape {
				...
				case 0:
					documents = append(
						documents,
						arena.Text(string(rune('a' + operation % 26))),
					)
				case 1:
				...
				case 4:
					documents[len(documents) - 1] = arena.Group(
						documents[len(documents) - 1],
					)
				case 5:
					documents[len(documents) - 1] = arena.Indent(
						documents[len(documents) - 1],
					)
				case 6:
					if len(documents) >= 2 {
						last := len(documents) - 1
						documents[last - 1] = arena.Concat(
							documents[last - 1],
							documents[last],
						)
				...
				case 7:
					if len(documents) >= 2 {
						last := len(documents) - 1
						documents[last - 1] = arena.IfBreak(
							documents[last - 1],
							documents[last],
						)
						documents = documents[:last]
				...
				case 8:
					documents = append(
						documents,
						arena.LineSuffix(arena.Text(" // suffix")),
					)
				case 9:
				...
				case 11:
					documents = append(
						documents,
						arena.SourceMarker(SourceMark{Offset: offset}),
					)
				case 12:
			...
			if first.Text != second.Text ||
				!slices.Equal(first.Markers, second.Markers) {
