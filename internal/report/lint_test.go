...
func TestNewLintResultEmitsStableVersionedDiagnostics(t *testing.T) {
	...
	diagnostic := rules.Diagnostic{
		...
		Related: []rules.Related{
			{Range: source.Range{Start: 15, End: 20}, Message: "owning function"},
		},
...
func TestNewLintResultRejectsMismatchedOrDuplicateSources(t *testing.T) {
	...
	mismatch := base
	mismatch.Diagnostics = []rules.Diagnostic{
		{Path: "/project/other.go", Digest: file.Digest()},
	}
	if _, err := NewLintResult("check", "findings", 1, true, []analysis.Result{mismatch}, nil);
		err == nil || !strings.Contains(err.Error(), "diagnostic source identity") {
		t.Fatalf("NewLintResult() mismatch error = %v", err)
	}
	if _, err := NewLintResult("check", "success", 0, true, []analysis.Result{base, base}, nil);
		err == nil || !strings.Contains(err.Error(), "duplicate source path") {
		t.Fatalf("NewLintResult() duplicate error = %v", err)
	}
	relative := base
	relative.Path = "source.go"
	if _, err := NewLintResult("check", "success", 0, true, []analysis.Result{relative}, nil);
	...
	if _, err := NewLintResult(
		"check",
		"success",
		0,
		true,
		[]analysis.Result{missingDigest},
		nil,
	);
...
func TestNewLintFixResultRejectsDiskStatusDigestContradictions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "unchanged with changed result",
			result: analysis.Result{Path: after.Path(), Digest: after.Digest()},
			status: LintFileUnchanged,
		},
		{
			name: "fixed with unchanged result",
		...
		t.Run(
			...
			func(t *testing.T) {
				...
				if _, err := NewLintFixResult(
					...
					err == nil || !strings.Contains(err.Error(), "digest") {
					t.Fatalf(
						"NewLintFixResult() error = %v, want digest contradiction",
						err,
					)
				}
			},
