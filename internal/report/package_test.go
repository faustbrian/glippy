...
func TestNewPackageLintResultMapsStableProblemChannels(t *testing.T) {
	...
	result, err := NewPackageLintResult(
		...
		analysis.PackageResult{
			...
			LoadDiagnostics: []analysis.PackageDiagnostic{
				{PackageID: "z/package", Position: "/project/z.go:2:3", Message: "undefined: z", Kind: packages.TypeError},
				{PackageID: "a/package", Position: "/project/a.go:1:1", Message: "expected declaration", Kind: packages.ParseError},
				{PackageID: "a/package", Position: "-", Message: "unclassified problem", Kind: packages.UnknownError},
			},
			SourceProblems: []analysis.PackageSourceProblem{
				{Path: "/project/z.go", Digest: problemDigest, Message: "invalid source z"},
				{Path: "/project/a.go", Digest: problemDigest, Message: "invalid source a"},
			},
		},
		nil,
	)
	...
	if result.Summary.PackageDiagnostics != 3 || result.Summary.SourceProblems != 2 ||
		len(result.PackageDiagnostics) != 3 || result.PackageDiagnostics[0].PackageID != "a/package" ||
		result.PackageDiagnostics[0].Kind != "unknown" || result.PackageDiagnostics[0].Position != "" ||
		result.PackageDiagnostics[1].Kind != "parse" || result.PackageDiagnostics[2].Kind != "type" ||
		len(result.SourceProblems) != 2 || result.SourceProblems[0].Path != "/project/a.go" ||
...
func TestRenderPackageLintTextOrdersDistinctProblemChannels(t *testing.T) {
	...
	output, err := RenderPackageLintText(
		...
		[]analysis.PackageSourceProblem{
			{Path: "/project/z.go", Digest: source.Digest{1}, Message: "invalid source z"},
			{Path: "/project/a.go", Digest: source.Digest{1}, Message: "invalid source a"},
		},
	)
	if err != nil {
		t.Fatal(err)
...
func TestPackageLintReportingRejectsInvalidProblemMetadata(t *testing.T) {
	...
	invalid := analysis.PackageResult{LoadDiagnostics: []analysis.PackageDiagnostic{{
		PackageID: "package", Message: "problem", Kind: packages.ErrorKind(99),
	}}}
	if _, err := NewPackageLintResult("check", "source-error", 2, false, invalid, nil); err == nil ||
		!strings.Contains(err.Error(), "unsupported package diagnostic kind") {
		t.Fatalf("NewPackageLintResult() error = %v", err)
	}
	if _, err := RenderPackageLintText(nil, nil, []analysis.PackageSourceProblem{{
		Path: "relative.go", Message: "problem",
	}}); err == nil || !strings.Contains(err.Error(), "normalized absolute") {
		t.Fatalf("RenderPackageLintText() error = %v", err)
	}
	if _, err := NewPackageLintResult("check", "source-error", 2, false, analysis.PackageResult{
		SourceProblems: []analysis.PackageSourceProblem{{Path: "/project/source.go", Message: "problem"}},
	}, nil); err == nil || !strings.Contains(err.Error(), "source digest") {
