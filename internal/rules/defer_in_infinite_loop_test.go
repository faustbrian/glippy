...
func (benchmarkDeferControlFlowRule) Metadata() rules.Metadata {
	return rules.Metadata{
		ID: "benchmark-defer-cfg",
		Summary: "provides a control-flow callback baseline",
...
`
	...
	for index, diagnostic := range result.Files[0].Diagnostics {
		...
		if diagnostic.RuleID != "defer-in-infinite-loop" ||
			...
			diagnostic.Range !=
				(source.Range{Start: offset, End: offset + len("defer")}) ||
...
func TestDeferInInfiniteLoopHonorsSuppressionGeneratedTypeErrorAndSeverityPolicies(t *testing.T) {
	...
	suppressed := runDeferInInfiniteLoop(
		t,
		`package sample
func cleanup() {}
func run() {
...
`,
	...
	generated := runDeferInInfiniteLoop(
		t,
...
`,
	...
	illTyped := runDeferInInfiniteLoop(
		t,
		`package sample
func cleanup() {}
...
`,
	...
	severity := runDeferInInfiniteLoop(
		t,
		`package sample
func cleanup() {}
func run() { for { defer cleanup() } }
...
func BenchmarkDeferInInfiniteLoopSharedCFG(b *testing.B) {
	...
	deferSelection, err := deferRegistry.Resolve(
