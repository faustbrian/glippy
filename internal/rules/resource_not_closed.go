...
func (resourceNotClosedRule) Metadata() Metadata {
	return Metadata{
		...
		KnownLimitations: []string{
			...
			"Cleanup-managed results require one stable direct local result, or bounded exact static delegation from one, plus an exact testing.T Cleanup call on a pointer receiver with a function-literal callback and direct, parameter-effect, or receiver-effect proven Close on every normally returning callback path. Recursive or dynamic delegation, copied testing.T values, conditional registration or closure, asynchronous or nested closure, reassignment, aliases, and non-testing cleanup APIs remain conservative.",
