...
func TestNilnessReportsOnlyProvenSSAFindings(t *testing.T) {
	...
	root, path := writeNilnessModule(t, `package sample
...
`)
	...
	for index, diagnostic := range result.Files[0].Diagnostics {
		...
		if !valid || text == "" {
			t.Fatalf("diagnostic %d range = %#v, text = %q, valid = %v", index, diagnostic.Range, text, valid)
		}
		if diagnostic.RuleID != "nilness" || diagnostic.Severity != rules.SeverityWarn || len(diagnostic.Fixes) != 0 {
...
func TestNilnessHonorsSuppressionGeneratedAndTypeErrorPolicies(t *testing.T) {
	...
	root, path := writeNilnessModule(t, `// Code generated by fixture. DO NOT EDIT.
package sample
...
`)
	...
	if len(result.Files) != 1 || result.Files[0].Path != path ||
		len(result.Files[0].Diagnostics) != 0 || len(result.Files[0].Suppressed) != 0 ||
		len(result.Files[0].UnusedSuppressions) != 0 || len(result.Files[0].SuppressionProblems) != 0 {
		t.Fatalf("generated result = %#v", result)
	}

	brokenRoot, _ := writeNilnessModule(t, `package sample
...
`)
	...
	if len(broken.LoadDiagnostics) == 0 || len(broken.Files) != 1 || len(broken.Files[0].Diagnostics) != 0 {
...
func TestNilnessSuppressionsAndSeverityUseCanonicalRuleIdentity(t *testing.T) {
	...
	root, _ := writeNilnessModule(t, `package sample
...
func TestNilnessIsOptInUntilEveryCommandSupportsTypedDefaults(t *testing.T) {
	t.Parallel()
	...
	if len(suspicious) != 1 || suspicious[0].ID != "nilness" ||
		suspicious[0].Requirement != rules.RequireSSA {
...
func BenchmarkNilnessSharedSSA(b *testing.B) {
	...
	nilnessSelection, err := nilnessRegistry.Resolve(rules.PresetSuspicious, nil)
	if err != nil {
		b.Fatal(err)
	}
	...
	b.Run("nilness", func(b *testing.B) {
